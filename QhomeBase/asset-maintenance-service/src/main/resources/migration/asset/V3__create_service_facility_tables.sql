...
CREATE TABLE IF NOT EXISTS asset.service (
    id                     BIGSERIAL PRIMARY KEY,
    category_id            BIGINT       NOT NULL REFERENCES asset.service_category(id) ON DELETE RESTRICT,
    ...
    booking_type           VARCHAR(50),                           -- COMBO_BASED, TICKET_BASED, OPTION_BASED, STANDARD
...
-- 3. Service availability (weekly schedule)
CREATE TABLE IF NOT EXISTS asset.service_availability (
    id             BIGSERIAL PRIMARY KEY,
    service_id     BIGINT      NOT NULL REFERENCES asset.service(id) ON DELETE CASCADE,
...
-- Option grouping (e.g., mandatory selections)
...
CREATE TABLE IF NOT EXISTS asset.service_option_group_item (
    id             BIGSERIAL PRIMARY KEY,
    group_id       BIGINT      NOT NULL REFERENCES asset.service_option_group(id) ON DELETE CASCADE,
    option_id      BIGINT      NOT NULL REFERENCES asset.service_option(id) ON DELETE CASCADE,
    sort_order     INTEGER     NOT NULL DEFAULT 0,
    created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
-- Combo composition (optional detailed breakdown)
CREATE TABLE IF NOT EXISTS asset.service_combo_item (
    id                  BIGSERIAL PRIMARY KEY,
    combo_id            BIGINT      NOT NULL REFERENCES asset.service_combo(id) ON DELETE CASCADE,
    included_service_id BIGINT      REFERENCES asset.service(id) ON DELETE SET NULL,
    option_id           BIGINT      REFERENCES asset.service_option(id) ON DELETE SET NULL,
...
-- 6. Tickets (time/entry-based products)
...
CREATE TABLE IF NOT EXISTS asset.service_booking (
    id                   BIGSERIAL PRIMARY KEY,
    service_id           BIGINT       NOT NULL REFERENCES asset.service(id) ON DELETE RESTRICT,
...
-- 8. Booking items (combo/option/ticket selections)
CREATE TABLE IF NOT EXISTS asset.service_booking_item (
    id            BIGSERIAL PRIMARY KEY,
    booking_id    BIGINT       NOT NULL REFERENCES asset.service_booking(id) ON DELETE CASCADE,
...
-- 9. Booking slots (resolved calendar slots per booking)
...
-- 10. KPI metric catalogue
CREATE TABLE IF NOT EXISTS asset.service_kpi_metric (
    id                 BIGSERIAL PRIMARY KEY,
    service_id         BIGINT REFERENCES asset.service(id) ON DELETE SET NULL,
    code               VARCHAR(100) NOT NULL,
    name               VARCHAR(200) NOT NULL,
    description        TEXT,
    unit               VARCHAR(50),
...
-- 11. KPI targets per period
CREATE TABLE IF NOT EXISTS asset.service_kpi_target (
    id                  BIGSERIAL PRIMARY KEY,
    metric_id           BIGINT      NOT NULL REFERENCES asset.service_kpi_metric(id) ON DELETE CASCADE,
    service_id          BIGINT REFERENCES asset.service(id) ON DELETE SET NULL,
